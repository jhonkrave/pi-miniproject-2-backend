...
const signToken = (userId) => {
    return jsonwebtoken_1.default.sign({ userId }, process.env.JWT_SECRET, { expiresIn: '2h' });
};
/**
 ...
 */
const loginRequestsByIp = new Map();
/**
...
router.post('/signup', async (req, res) => {
    try {
        ...
        const existingByEmail = await UserDAO_1.default.findOne({ email });
        if (existingByEmail) {
            return res.status(409).json({ message: 'Email already exists' });
        }
        const hashed = await bcryptjs_1.default.hash(password, 10);
        const user = await UserDAO_1.default.create({ email, password: hashed, firstname, lastname, age: numericAge });
...
router.post('/login', loginRateLimit, async (req, res) => {
    try {
        ...
        if (!ok) {
            ...
            await UserDAO_1.default.update(user._id, { loginAttempts: attempts, lockUntil });
            return res.status(attempts >= 5 ? 423 : 401).json({ message: attempts >= 5 ? 'Account locked. Try later' : 'Invalid credentials' });
        }
        if (user.loginAttempts || user.lockUntil) {
            await UserDAO_1.default.update(user._id, { loginAttempts: 0, lockUntil: null });
        }
...
router.post('/password/forgot', async (req, res) => {
    try {
        ...
        await UserDAO_1.default.update(user._id, { resetPasswordToken: token, resetPasswordExpires: expires });
        ...
        if (isDev) {
            console.log('[forgot] smtpConfigured:', (0, mailer_1.isSmtpConfigured)());
            console.log('[forgot] queue email with token:', token.slice(0, 6) + '…');
        }
        Promise.resolve()
            .then(() => (0, mailer_1.sendPasswordResetEmail)({ to: email, token }))
            .then(() => { if (isDev)
            console.log('[forgot] email send completed'); })
            .catch((mailErr) => {
...
router.post('/password/reset', async (req, res) => {
    try {
        ...
        if (!passwordRegexReset.test(newPassword))
            return res.status(400).json({ message: 'Password does not meet complexity requirements' });
        const user = await UserDAO_1.default.findOne({ resetPasswordToken: token, resetPasswordExpires: { $gt: new Date() } });
        if (!user)
            return res.status(400).json({ message: 'Invalid or expired token' });
        const hashed = await bcryptjs_1.default.hash(newPassword, 10);
        await UserDAO_1.default.update(user._id, { password: hashed, resetPasswordToken: null, resetPasswordExpires: null });
...
router.get('/password/verify', async (req, res) => {
    try {
        ...
        const user = await UserDAO_1.default.findOne({ resetPasswordToken: token, resetPasswordExpires: { $gt: new Date() } });
        if (!user)
            return res.status(200).json({ valid: false, message: 'Enlace inválido o caducado' });
 ...
 * @throws {500} Internal server error
 */
router.get('/users/me', authMiddleware_1.default, async (req, res) => {
    try {
        const user = await UserDAO_1.default.findOne({ _id: req.userId });
        if (!user)
 ...
 * @throws {500} Internal server error
 */
router.put('/users/me', authMiddleware_1.default, async (req, res) => {
    try {
        ...
        const existingUser = await UserDAO_1.default.findOne({ email, _id: { $ne: userId } });
        if (existingUser) {
            return res.status(409).json({ message: 'Email already in use' });
        }
        // Prepare update object
        const updateData = { firstname, lastname, age: numericAge, email };
        // Add password to update if provided
        if (password) {
            const hashedPassword = await bcryptjs_1.default.hash(password, 10);
            updateData.password = hashedPassword;
        }
        const updatedUser = await UserDAO_1.default.update(userId, updateData);
        const userResponse = {
            id: updatedUser._id,
 ...
 * @throws {500} Internal server error
 */
router.delete('/users/me', authMiddleware_1.default, async (req, res) => {
    try {
        ...
        const user = await UserDAO_1.default.findOne({ _id: userId });
        if (!user) {
            return res.status(404).json({ message: 'Account not found' });
        }
        await UserDAO_1.default.delete(userId);
        res.clearCookie('token', { ...cookieOptions, maxAge: undefined });
        res.status(204).send();
        return;
    }
...
router.get('/health', async (req, res) => {
    try {
        const isSmtp = await (0, mailer_1.testSmtpConnection)();
